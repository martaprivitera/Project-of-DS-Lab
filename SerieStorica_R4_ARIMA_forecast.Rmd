---
title: "Ristorante 4 - Modelli Arima"
date: "2023-11-08"
...
print(confronti1)
```
## Filtro la serie temporale a partire da giugno.
...
## Plot della serie temporale con frequenza settimanale

```{r}
time_series <- ts(dtts_june_2021, frequency=7)
plot(time_series)
```
# Faccio una trasformazione logaritmica e poi faccio un grafico dell'Acf e della Pacf

```{r}
time_series_new<-log(time_series)
time_series_new |> Acf(70)
...
train_size <- floor(length(time_series_new[1:579]))
...
plot(pred_values, type = "l", col = "red", lty = 1, lwd = 2, ylim = range(c(pred_values, test_y)))
lines(length(train_y) + 1:length(test_y), test_y, col = "blue", type = "l", lty = 2, lwd = 2)
legend("topright", legend = c("Forecast", "Valori Reali"), col = c("red", "blue"), lty = 1:2, lwd = 0.001)
