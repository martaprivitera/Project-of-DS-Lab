---
title: "Modelli di Machine Learning per il ristorante 5"
date: "2023-11-08"
...
\
Leggo il csv del ristorante R5:\
...
# Crea la serie storica con le date nel formato "YYYY-MM-DD" e i valori da LT_R5
dtts <- xts(
  dt$LT_R5, # Selezioniamo la colonna LT_R5 come dati sull'asse delle y
...
dtts_may_2021 <- dtts["2021-05-01/"]
plot(dtts_may_2021)
```

## Inizio la costruzione dei modelli:

```{r}
#Carico la serie temporale
time_series <- ts(dtts_may_2021)
#Dataset di train e di validation 
train <- dtts_may_2021["2021-05-01/2022-12-31"]
val <- dtts_may_2021["2023-01-01/2023-05-03"]
```

```{r}
#Converto in formato ts train e validation set e ne guardo le dimensioni:
ts_train =ts(train)
ts_val = ts(val)
dim(ts_train)
dim(ts_val)
...
dt1 <- dt1[-c(31, 92, 123, 184, 245, 396, 457, 488, 549, 610, 731, 732, 733),]
...
center_history <- rec_obj$steps[[2]]$means["LT_R5"]
scale_history  <- rec_obj$steps[[3]]$sds["LT_R5"]
...
y_train <- array(data = train_lag$LT_R5, dim = c(length(train_lag$LT_R5), tsteps))
...
y_val <- array(data = val_lag$LT_R5, dim = c(length(val_lag$LT_R5), tsteps))
```
...
#Indici
MAE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R5))
MAPE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R5) / val_lag$LT_R5) * 100
ME_lstm1 <- mean(pred_lstm1$value - val_lag$LT_R5)
RMSE_lstm1 <- sqrt(mean((pred_lstm1$value - val_lag$LT_R5)^2))
MPE_lstm1 <- mean((pred_lstm1$value - val_lag$LT_R5) / val_lag$LT_R5) * 100
...
```{r}
ggplot() +
  autolayer(ts(df_val$LT_R5), series="Validation",size=0.4) +
...
#Indici
MAE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R5))
MAPE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R5) / val_lag$LT_R5) * 100
ME_gru1 <- mean(pred_gru1$value - val_lag$LT_R5)
RMSE_gru1 <- sqrt(mean((pred_gru1$value - val_lag$LT_R5)^2))
MPE_gru1 <- mean((pred_gru1$value - val_lag$LT_R5) / val_lag$LT_R5) * 100
