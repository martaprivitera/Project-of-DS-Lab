---
title: "Ristorante 2-Serie Storica"
author: "Eleonora Brambatti"
date: "2023-12-27"
...
ora iniziamo a dividere la serie storica in anni
\
```{r}
...
time_series <- ts(dtts_july_2021)
train <- dtts_july_2021["2021-06-01/2022-12-31"]
val <- dtts_july_2021["2023-01-01/2023-05-03"]
...
\
```{r}
mape_gru2 <- mean(abs((val[sequence_length:length(val)] - predicted_val_gru) / val[sequence_length:length(val)]) * 100, na.rm = TRUE)
rmse_gru2 <- sqrt(mean((predicted_val_gru - val[sequence_length:length(val)])^2))
mae_gru2 <- mean(abs(predicted_val_gru - val[sequence_length:length(val)]))
mpe_gru2 <- mean((predicted_val_gru - val[sequence_length:length(val)]) / val[sequence_length:length(val)]) * 100
me_gru2 <- mean(predicted_val_gru - val[sequence_length:length(val)])
