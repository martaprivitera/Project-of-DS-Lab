---
title: "Modelli di Machine Learning per il ristorante 4"
date: "2023-11-08"
...
\
Leggo il csv del ristorante R4:\
...
# Crea la serie storica con le date nel formato "YYYY-MM-DD" e i valori da LT_R4
dtts <- xts(
  dt$LT_R4, # Selezioniamo la colonna LT_R4 come dati sull'asse delle y
...
dtts_june_2021 <- dtts["2021-06-01/"]
plot(dtts_june_2021)
```
...
train <- dtts_june_2021["2021-06-01/2022-12-31"]
val <- dtts_june_2021["2023-01-01/2023-05-03"]
```

\
Creo dataset di train e di test e ne guardo le dimensioni:\
...
dt2 <- dt1[-c(61, 92, 153, 214, 365, 426, 457, 518, 579, 700, 701, 702),]

# Verifica le versioni dei pacchetti
packageVersion("dplyr")
packageVersion("tibble")

# suddivido in train e validation
df_train <- dt2[1:570,] 
df_val <- dt2[571:690,] 
...
# Faccio Scaling e Centering del dataset
rec_obj <- recipe(LT_R4 ~ ., df) %>%
    step_sqrt(LT_R4) %>%
    step_center(LT_R4) %>%
    step_scale(LT_R4) %>%
    prep()
...
center_history <- rec_obj$steps[[2]]$means["LT_R4"]
scale_history  <- rec_obj$steps[[3]]$sds["LT_R4"]
...
y_train <- array(data = train_lag$LT_R4, dim = c(length(train_lag$LT_R4), tsteps))
...
y_val <- array(data = val_lag$LT_R4, dim = c(length(val_lag$LT_R4), tsteps))
```
...
#Indici
MAE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R4))
MAPE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R4) / val_lag$LT_R4) * 100
ME_lstm1 <- mean(pred_lstm1$value - val_lag$LT_R4)
RMSE_lstm1 <- sqrt(mean((pred_lstm1$value - val_lag$LT_R4)^2))
MPE_lstm1 <- mean((pred_lstm1$value - val_lag$LT_R4) / val_lag$LT_R4) * 100
...
```{r}
ggplot() +
  autolayer(ts(df_val$LT_R4), series="Validation",size=0.4) +
...
#Indici
MAE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R4))
MAPE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R4) / val_lag$LT_R4) * 100
ME_gru1 <- mean(pred_gru1$value - val_lag$LT_R4)
RMSE_gru1 <- sqrt(mean((pred_gru1$value - val_lag$LT_R4)^2))
MPE_gru1 <- mean((pred_gru1$value - val_lag$LT_R4) / val_lag$LT_R4) * 100
