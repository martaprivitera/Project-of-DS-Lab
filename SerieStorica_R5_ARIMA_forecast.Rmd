---
title: "Ristorante 5 - Modelli Arima"
date: "2023-11-08"
...
```{r}
time_series <- ts(dtts_may_2021, frequency=7)
plot(time_series)
```
...
train_size <- floor(length(time_series_new[1:500]))
...
# Parametri per la stagionalità
s <- 365.25
harm <- c(1,2,3) # passo da 3 a 2 sinusoidi, altrimenti mi produce dei nans
freq <- outer(1:length(time_series_new), harm) * 2 * pi / s
co <- cos(freq)
si <- sin(freq)
colnames(co) <- paste0("cos", harm)
...
# Divisione in set di addestramento e test
train_size <- floor(length(time_series_new[1:610]))
train_y <- time_series_new[1:train_size]
train_X <- X[1:train_size, ]
test_y <- time_series_new[(train_size + 1):length(time_series_new)]
test_X <- X[(train_size + 1):length(time_series_new), ]
