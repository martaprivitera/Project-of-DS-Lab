---
title: "Modelli di Machine Learning per il ristorante 2"
date: "2023-10-30"
...
Leggo il csv del ristorante R2:

```{r}
R2 <- read.csv("R2.csv")

R2$Date <- as.Date(R2$data, format = "%Y-%m-%d")

plot(R2$Date, R2$S_R2, xlab = "Data", ylab = "Scontrini", main = "Grafico Scontrini vs. Data", type = "l")
```

\
Scatterplot ristorante R2:

```{r}
plot(R2$S_R2, R2$LT_R2, xlab = "Scontrini", ylab = "Lordo Totale", main = "Grafico Lordo Totale vs. Scontrini")
```

```{r}
plot(R2$Date, R2$LT_R2, xlab = "Data", ylab = "Lordo Totale", main = "Grafico Lordo Totale vs. Data", type = "l")
```

\
Ora considero solo le colonne lt e data e creo un altro dataset\

```{r}
R2_LT <- xts(
  R2$LT_R2, # Selezioniamo la colonna LT_R1 come dati sull'asse delle y
  R2$Date # Utilizziamo la colonna "Date" formattata come oggetti di data sull'asse delle x
)
...
R2_2018 <- R2_LT["2018-01-01/2018-12-31"]

# Filtra la serie storica per il 2019
R2_2019 <- R2_LT["2019-01-01/2019-12-31"]

# Filtra la serie storica per il 2021
R2_2021 <- R2_LT["2021-01-01/2021-12-31"]

# Filtra la serie storica per il 2022
R2_2022 <- R2_LT["2022-01-01/2022-12-31"]

# Filtra la serie storica per il 2022
R2_2023 <- R2_LT["2023-01-01/2023-12-31"]
```

```{r}
# Calcola la media mensile per il 2018
monthly_mean_2018 <- apply.monthly(R2_2018, FUN = mean)

# Calcola la media mensile per il 2019
monthly_mean_2019 <- apply.monthly(R2_2019, FUN = mean)

# Calcola la media mensile per il 2019
monthly_mean_2021 <- apply.monthly(R2_2021, FUN = mean)

# Calcola la media mensile per il 2019
monthly_mean_2022 <- apply.monthly(R2_2022, FUN = mean)

# Calcola la media mensile per il 2019
monthly_mean_2023 <- apply.monthly(R2_2023, FUN = mean)
```
...
Ora concateniamo i due dataset:
...
Facciamo il cut della serie storica sulla base della tolleranza decisa sopra\

```{r}
# Filtra la serie storica a partire da luglio 2021
dtts_july_2021 <- R2_LT["2021-07-01/"]
plot(dtts_july_2021)
```

## Inizio costruzione dei modelli

```{r}
# Carica i pacchetti

time_series <- ts(dtts_july_2021)

train <- dtts_july_2021["2021-07-01/2022-12-31"]
val <- dtts_july_2021["2023-01-01/2023-05-03"]
```

```{r}
#SUddivido in train e test set e guardo le dimensioni:
ts_train =ts(train)
ts_val = ts(val)
dim(ts_train)
dim(ts_val)
...
R2_2 <- R2_1[-c(31, 62, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 164, 315, 376, 407, 468, 529),]
...
center_history <- rec_obj$steps[[2]]$means["LT_R2"]
scale_history  <- rec_obj$steps[[3]]$sds["LT_R2"]
...
y_train <- array(data = train_lag$LT_R2, dim = c(length(train_lag$LT_R2), tsteps))
...
y_val <- array(data = val_lag$LT_R2, dim = c(length(val_lag$LT_R2), tsteps))
```
...
#Indici
MAE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R2))
MAPE_lstm1 <- mean(abs(pred_lstm1$value - val_lag$LT_R2) / val_lag$LT_R2) * 100
ME_lstm1 <- mean(pred_lstm1$value - val_lag$LT_R2)
RMSE_lstm1 <- sqrt(mean((pred_lstm1$value - val_lag$LT_R2)^2))
MPE_lstm1 <- mean((pred_lstm1$value - val_lag$LT_R2) / val_lag$LT_R2) * 100
...
```{r}
ggplot() +
  autolayer(ts(df_val$LT_R2), series="Validation",size=0.4) +
...
MAE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R2))
MAPE_gru1 <- mean(abs(pred_gru1$value - val_lag$LT_R2) / val_lag$LT_R2) * 100
ME_gru1 <- mean(pred_gru1$value - val_lag$LT_R2)
RMSE_gru1 <- sqrt(mean((pred_gru1$value - val_lag$LT_R2)^2))
MPE_gru1 <- mean((pred_gru1$value - val_lag$LT_R2) / val_lag$LT_R2) * 100
