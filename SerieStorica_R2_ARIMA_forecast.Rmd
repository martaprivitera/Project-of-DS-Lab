---
title: "Ristorante 2 - Modelli Arima"
date: "2023-10-30"
...
```{r}
time_series <- ts(dtts_july_2021, frequency=7)
plot(time_series)
```
...
acf_result <- Acf(time_series_new)


pacf_result <- Pacf(time_series_new)
...
train_size <- floor(length(time_series_new[1:529]))
...
# Parametri per la stagionalità
s <- 365.25
harm <- c(1,2,3) # passo da 2 sinusoidi a 1 perché altrimenti vengono prodotti valori mancanti
freq <- outer(1:length(time_series_new), harm) * 2 * pi / s
co <- cos(freq)
si <- sin(freq)
colnames(co) <- paste0("cos", harm)
