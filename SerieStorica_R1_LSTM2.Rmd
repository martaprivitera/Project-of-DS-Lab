...
```{r}
mape_lstm2 <- mean(abs((val - predicted_val) / val) * 100, na.rm = TRUE)
rmse_lstm2 <- sqrt(mean((predicted_val - val)^2))
mae_lstm2 <- mean(abs(predicted_val - val))
mpe_lstm2 <- mean((predicted_val - val) / val) * 100
me_lstm2 <- mean(predicted_val - val)
