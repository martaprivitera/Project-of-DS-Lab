---
title: "Ristorante 1 - Modelli Arima"
...
print(confronti1)
```
Il seguente cut della serie temporale viene eseguito partendo dale considerazioni fatte in merito alla variabile 'scontrini' del dataset. Non potendo considerare il 2021 come anno post pandemia abbiamo deciso di eseguire il cut a partire dal 2022 (considerato invece periodo post pandemia).

```{r}
# Filtra la serie storica a partire da gennaio 2022
dtts_2022 <- dtts["2022-01-01/"]
plot(dtts_2022, ylab = "Daily Revenues", xlab = "Time", main = "Daily Revenue Time Series Plot")

```
```{r}
#time_series<- dtts_june_2021
time_series <- ts(dtts_2022, frequency=7)
```


Trasformazioni per stazionarizzare il modello:
...
```{r}
# Fit del modello SARIMA sulla serie logaritmica
mod_sarima <- auto.arima(log(time_series))
...
shapiro_test <- shapiro.test(residuals(mod_sarima))
print(shapiro_test)
```
\
...
s <- 365.25
harm <- c(1,2) #passo da 3 a 2 perché se no mi produce Nans
...
train_size <- floor(length(time_series_new[1:455]))
...
plot(pred_values, type = "l", col = "red", lty = 1, lwd = 2, ylim = range(c(pred_values, test_y)), ylab = "Yield revenues in thousands", xlab = "Days")
# Aggiungi titolo
title(main = "Actual vs Predicted revenue amount per day")

lines(length(train_y) + 1:length(test_y), test_y, col = "blue", type = "l", lty = 2, lwd = 2)

# Aggiungi legenda fuori dal grafico con carattere più piccolo
legend("bottomright", legend = c("Predicted values", "Actual values"), col = c("red", "blue"), lty = c(1, 2), lwd = 2, bty = "n", cex = 0.7)
...
```{r}
# Plot dei residui con aggiunta di linee tratteggiate
plot(mod_sin$residuals, ylab = "Residual values", xlab = "Days",
     main = "Residual Values for ARIMAX 2 model with sinusoids")

# Aggiungi linee tratteggiate in rosso a 0.05 e -0.05
...
dove dovrebbero essere le dummy:
 ...
 208 (natale), 321 (pasqua), 322 (lunedì dell'angelo), 339 (vicino a ferragosto), 518 (halloween), 573 (natale), 574 (santo stefano), 579 (Capodanno)
...
# Esegui il forecast
forecast_result <- forecast(mod_sin_dummy , h = 7, xreg = test_X)

# Plot del risultato
plot(forecast_result, ylab = "Revenues values", xlab = "Days",
     main = "Forecasts results for ARIMA (2,0,0) with sinusoids and dummy variables")
...
# Plot dei residui con aggiunta di linee tratteggiate
plot(mod_sin_dummy$residuals, ylab = "Residual values", xlab = "Days",
     main = "Residual Values for ARIMAX 2 model with dummy variables")
...
# Parametri per la stagionalità
s <- 365.25
harm <- c(1,2) # passo da 3 a 2 sinusoidi, altrimenti mi produce dei nans
freq <- outer(1:length(time_series_new), harm) * 2 * pi / s
co <- cos(freq)
si <- sin(freq)
colnames(co) <- paste0("cos", harm)
...
dove dovrebbero essere le variabili dummy:
...
## Modello Arima 5 con sinusoidi come regressori
